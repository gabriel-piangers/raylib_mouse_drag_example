/*******************************************************************************************
*
*   raylib [core] example - mouse drag and drop
*
...
int main(void)
{
    ...
    InitWindow(screenWidth, screenHeight, "raylib [core] example -  mouse drag and drop");
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (RecCol.width == rec.width && RecCol.height == rec.height) {
            recPlaced = true;
        }        
        ...
        if (Vector2Distance(circ.center, circArea.center) < circArea.radius - circ.radius) {
            circPlaced = true;
        }
